...
#     c_order   = biaya_order * (init_T / (max(purchases_freq, 1) * init_R))
#     c_hold    = product["biaya_simpan"] * round((init_S + init_s) / 2)

#     # Use actual stockout units × shortage cost — no integral approximation
#     c_stockout = product["biaya_kekurangan"] * total_stockout

#     c_total   = c_order + c_hold + c_stockout

#     # Secondary penalty to strongly discourage stockout
...
def min_fitness(product, demand, init_R, init_s, init_S, init_T, purchases_freq, tot_lost, stockout_weight=1.0):
    ...
    result = evaluate_policy(product, demand, init_R, init_s, init_S, init_T)

    # weight inventory > stockout
    fitness = (
        result["c_order"] +
        result["c_hold"] +
        result["c_stockout"] * 0.35
    )

    return fitness, result["stockout"]
...
# ---------------------------------------------------------------------------
def log_scaled_mutation(individual, mutation_rate, sigma=0.1, lower_bound=1, upper_bound=1000, max_gap=1000):
...
def fix_S_s(individual, max_gap=1000):
    ...
    max_S = int(mean_d * (R + lead) * 1.3 + safety)
...
# ---------------------------------------------------------------------------
def genetic_algorithm(product_data, population_size, num_generations, crossover_rate, mutation_rate, daily_sales, daily_purchases, stockout_weight=1.0):
    """
...
def run_with_pso(product_data, daily_sales, daily_purchases, user_pop_size, user_num_gen, user_cr, user_mr, use_pso=True, pso_n_particles=PSO_N_PARTICLES, pso_n_iters=PSO_N_ITERS, stockout_weight=PSO_STOCKOUT_WEIGHT):
    ...
    ga_result = genetic_algorithm(product_data, final_pop, final_gen, final_cr, final_mr, daily_sales, daily_purchases, stockout_weight=stockout_weight)
        ...
        if request.user.employee.role == 'superadmin':
            ...
            for outlet in outlets:
                ...
                for item_index, item in enumerate(items):
                    ...
                    standar_deviasi = (np.std(sales_list_raw) if len(sales_list_raw) > 1 else (sales_list_raw[0] if sales_list_raw else 1))
                    ...
                    # first_c_hold    = (biaya_simpan * ((first_S + first_s) / 2) + (first_total_demand * first_R) / fp_freq)
                    ...
                    # first_c_stockout = product["biaya_kekurangan"] * first_total_so
                    first_c_total     = first_c_order + first_c_hold + first_c_stockout
                    first_calc_duration += time.time() - temp_first_start

                    result = evaluate_policy(product, first_demand, first_R, first_s, first_S, first_T)

                    first_c_order = result['c_order']
                    first_c_hold = result['c_hold']
                    first_c_stockout = result['c_stockout']
                    first_c_total = result ['c_total']
                    ...
                    # c_hold   = (biaya_simpan * round((best_S + best_s) / 2) + round((tot_demand_best * best_R) / mod_pf))
                    ...
                    # c_stockout = product["biaya_kekurangan"] * total_so
                    c_total    = c_order + c_hold + c_stockout

                    result = evaluate_policy(product, best_demand, best_R, best_s, best_S, best_T)

                    c_order = result['c_order']
                    c_hold = result['c_hold']
                    c_stockout = result['c_stockout']
...
def evaluate_policy(product, demand, R, s, S, T):
    ...
    total_stockout = sum(lost_list)

    biaya_order = product["biaya_order"]
    ...
    mean_daily = np.mean(half_demand) if half_demand else 0
    std_daily = np.std(half_demand, ddof=1) / np.sqrt(T) if len(half_demand) > 1 else 1e-9
    total_daily_dmd = round(sum(half_demand) / T) if T > 0 else 0

    # ORDER COST
    fp_freq         = max(purchases_freq, 1)
    c_order = biaya_order * (T / (fp_freq * R))
    # c_order = biaya_order * max(purchases_freq,1)

    # HOLDING COST
    c_hold  = product["biaya_simpan"] * round((S + s) / 2)

    # REAL STOCKOUT COST
    # c_stockout = biaya_kurang * total_stockout
    ...
    total_cost = c_order + c_hold + c_stockout

    return {
        "c_order": c_order,
        "c_hold": c_hold,
        "c_stockout": c_stockout,
        "c_total": total_cost,
