...
# Jumlah evaluasi BO
BO_N_CALLS     = 50   # total evaluasi (termasuk n_initial_points)
BO_N_INITIAL   = 10    # titik acak awal sebelum GP mulai memodelkan

BO_STOCKOUT_WEIGHT = 1.0
...
def log_scaled_mutation(individual, mutation_rate, sigma=0.1, lower_bound=1, upper_bound=10000):
    ...
    for i, gene in enumerate(mutated):
        # Skip product, demand, T, purchases_freq, tot_lost
        if i in [0, 1, 5, 6, 7]:
            continue
...
def genetic_algorithm(product_data, population_size, num_generations,
        ...
        daily_sales, daily_purchases, stockout_weight=1.0):
    """
...
def periodic_view(request):
    if request.method == 'POST':
        ...
        for x in array:
            ...
            plt.hist(demand_result_filtered, color="#70AD47")
            plt.xlabel('Demand')
            ...
            plt.figure(figsize=(6, 4))
            plt.hist(inventory_cost_list, color="#70AD47")
        ...
        if request.user.employee.role == 'superadmin':
            ...
            if use_bo:
                ...
                for outlet in outlets:
                    ...
                    for item in items:
                        try:
                            outlet_item = OutletItem.objects.filter(
                                outlet=outlet.id,
                                item=item.id
                            ).first()
                            ...
                            bo_sales_qs = (Sales.objects.filter(outlet_id=outlet.id, item_id=item.id).values('created_at').annotate(total_sales=Sum('amount')))

                            bo_purch_qs = (Purchase.objects.filter(outlet_id=outlet.id, item_id=item.id).values('created_at').annotate(total_purchases=Sum('amount')))

                            bo_sales_dict = {s['created_at'].date(): s['total_sales']     for s in bo_sales_qs}
                            bo_purch_dict = {p['created_at'].date(): p['total_purchases'] for p in bo_purch_qs}

                            if not bo_sales_dict:
                                continue

                            start_date = min(bo_sales_dict.keys())
                            end_date = start_date + timedelta(days=59)
                            ...
                            cur = start_date
                            while cur <= end_date:
                                daily_sales_bo.append(bo_sales_dict.get(cur, 0))
                                daily_purchases_bo.append(bo_purch_dict.get(cur, 0))
                                cur += timedelta(days=1)

                            total_sales_bo = sum(daily_sales_bo)
                            std_bo = np.std(daily_sales_bo, ddof=1) if len(daily_sales_bo) > 1 else 1

                            product_bo = {
                                'nama_barang':      item.name,
                                'biaya_pesan':      item.biaya_pesan,
                                'biaya_order':      biaya_order,
                                'permintaan_baku':  total_sales_bo if total_sales_bo > 0 else 1,
                                'biaya_simpan':     biaya_simpan,
                                'biaya_kekurangan': round((item.price * 7.5 / 100) + item.price),
                                'harga_produk':     item.price,
                                'lead_time':        outlet_item.lead_time / 100,
                                'standar_deviasi':  std_bo,
                            }

                            products_data.append((product_bo, daily_sales_bo, daily_purchases_bo))

                        except Exception as e:
                            print(f"BO skip outlet={outlet.id}, item={item.id}: {e}")
                            continue
    ...
    if request.method == 'POST':
        ...
        if request.user.employee.role == 'superadmin':
            ...
            for x in array:
                ...
                # After processing all products for this outlet, generate the plot
                fig, ax = plt.subplots(figsize=(18, 6))
                ax.plot(combined_inventory_level, linewidth=1.5, color="#70AD47")
...
def _bo_evaluate(params, product_data, daily_sales, daily_purchases, stockout_weight=BO_STOCKOUT_WEIGHT):
    ...
    try:
        ...
        # result[16] is already the composite score
